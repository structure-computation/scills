...
void calcul_plasticite(Sst &S, Process &process) {
    ...
    const TYPEREEL mu = S.matprop.elastic_modulus/(2.*(1.+S.matprop.poisson_ratio));    /// Second coefficient de Lame
    ...
    for(unsigned i_elem = 0; i_elem < S.mesh.elem_list_size;i_elem++){
        ...
        Vec<TYPEREEL,DIM*(DIM+1)/2> alpha_elas = all_sigma[i_elem];
