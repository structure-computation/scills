...
/** \ingroup
\brief Fonction principale pour un calcul sous-structur�
*/
    ...
    if(process.read_data==1) {
      if (process.rank == 0) std::cout << "Allocations des quantites d'interfaces et SST" << std::endl;
        ...
        if(process.save_data==1) {
          if (process.rank == 0) std::cout << "Sauvegarde des r�sultats dans les fichiers save_sst et save_inter" << std::endl;
