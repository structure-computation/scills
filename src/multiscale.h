...
void multiscale_calculation(DataUser &data_user,
    ...
    if(data_user.options.Multiresolution_on==1 and data_user.options.Multiresolution_material_link_CL_CLvolume[0]==1){
        if (process.parallelisation->is_master_cpu()) std::cout << "Reevaluation (multiresolution) des materiaux SST : " ;
    ...
    //process.temps->dt=0;
    if (process.parallelisation->is_master_cpu)  std::cout << "Nombre de pas de temps total " << process.temps->nbpastemps << std::endl;
    ...
    for(unsigned i_step=0;i_step<process.temps->nb_step;i_step++){
        ...
        if(process.temps->dt!=process.temps->time_step[i_step].dt or calculate_operator==1){
            ...
            if (process.parallelisation->is_master_cpu) std::cout << "******************************************" << std::endl;
            if (process.parallelisation->is_master_cpu) std::cout << " Construction des operateurs" << std::endl;
            if (process.parallelisation->is_master_cpu) std::cout << "******************************************" << std::endl;
            ...
            process.parallelisation->synchronisation();
            if (process.parallelisation->is_master_cpu()){
                std::cout << "Construction operateurs : " ;
                tic1.stop();
                std::cout << std::endl;
                tic1.start();
            }
    ...
    if(process.read_data==1) {
      if (process.parallelisation->is_master_cpu) std::cout << "Allocations des quantites d'interfaces et SST" << std::endl;
        ...
        if (process.parallelisation->is_master_cpu) std::cout <<  std::endl;
        if (process.parallelisation->is_master_cpu) std::cout << "**************************" << std::endl;
        if (process.parallelisation->is_master_cpu) std::cout << " DEBUT DU CALCUL ITERATIF "<< std::endl;
        if (process.parallelisation->is_master_cpu) std::cout << "**************************" << std::endl;

        /// calcul iteratif
        if (process.nom_calcul=="latin") {
            if (process.parallelisation->is_master_cpu) std::cout << "Calcul latin" << std::endl;
            multiscale_iterate_latin(S,SubS,Inter, SubI,process, Global,CL, data_user);
        } else if(process.nom_calcul=="incr") {
            if (process.parallelisation->is_master_cpu) std::cout << "Calcul incremental" << std::endl;
            multiscale_iterate_incr(S,SubS, Inter,SubI,process, Global,CL, data_user, geometry_user, field_structure_user);
        } else {
            if (process.parallelisation->is_master_cpu) std::cout << "nom de calcul non defini : choix entre latin ou incremental" << std::endl;
            assert(0);
        }
...
//             save_data_sst(SubS, process, fields_to_save2);
    ...
    process.parallelisation->synchronisation();
    if (process.parallelisation->is_master_cpu()){
        std::cout << "Duree de la partie iterative : " ;
        tic1.stop(); 
        std::cout << std::endl;
        tic1.start();
    }
...
void multiscale(DataUser       &data_user,
    ...
    if (process.parallelisation->is_master_cpu){
        std::cout <<  std::endl;
        std::cout << "******************************************" << std::endl;
        std::cout << " Construction du maillage micro et macro" << std::endl;
        std::cout << "******************************************" << std::endl;
    }
    ...
    process.parallelisation->synchronisation();
    if (process.parallelisation->is_master_cpu()){
        std::cout << "Construction maillages : " ;
        tic1.stop();
        std::cout << std::endl;
        tic1.start();
    }
...
//     }
    ...
    #ifdef INFO_TIME
        process.parallelisation->synchronisation();
        if (process.parallelisation->is_master_cpu()) std::cout << "Assignation materiaux SST : " ;
        if (process.parallelisation->is_master_cpu()) tic1.stop();
        if (process.parallelisation->is_master_cpu()) std::cout << std::endl;
    ...
    #ifdef INFO_TIME
        process.parallelisation->synchronisation();
        if (process.parallelisation->is_master_cpu()) std::cout << "Assignation materiaux INTER : " ;
        if (process.parallelisation->is_master_cpu()) tic1.stop();
        if (process.parallelisation->is_master_cpu()) std::cout << std::endl;
        if (process.parallelisation->is_master_cpu()) tic1.start();
    #endif
...
//     }
    ...
    if (process.parallelisation->is_master_cpu) std::cout << "Sauvegarde de la geometrie du maillage de peau au format hdf pour la visualisation des resultats" << std::endl;
...
#ifdef INFO_TIME
    process.parallelisation->synchronisation();
    if (process.parallelisation->is_master_cpu()) std::cout << "Affichage maillage : " ;
    if (process.parallelisation->is_master_cpu()) std::cout << std::endl;
    if (process.parallelisation->is_master_cpu()) tic1.start();
#endif
