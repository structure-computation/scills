...
struct GeometryCompactClass {
    ...
    struct GroupElementsCompactClass {
        ...
        template<class T> static GroupElementsCompactClass *copy( MemoryDriver &md, const T *src, ST num = 1 ) {
            ...
            for( ST i = 0; i < num; ++i ) {
                ...
                BasicVecRef<SI32 > *loc_0;
                ST rese_0 = src[ i ].connectivities.size() * sizeof( BasicVecRef<SI32 > );
                md.beg_local_data( &loc[ i ].connectivities.data_, &loc_0, rese_0, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].connectivities.size(); ++j ) {
                    loc_0[ j ].size_ = src[ i ].connectivities[ j ].size();
                    loc_0[ j ].rese_ = src[ i ].connectivities[ j ].size();
                    md.copy( &loc_0[ j ].data_, src[ i ].connectivities[ j ].ptr(), src[ i ].connectivities[ j ].size() * sizeof( SI32 ), 16 * 4 );
                }
                md.end_local_data( loc[ i ].connectivities.data_, loc_0, rese_0 );
                ...
                ST rese_1 = src[ i ].interface_group_id.size() * sizeof( BasicVecRef<SI32 > );
                md.beg_local_data( &loc[ i ].interface_group_id.data_, &loc_1, rese_1, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].interface_group_id.size(); ++j ) {
                    loc_1[ j ].size_ = src[ i ].interface_group_id[ j ].size();
                    loc_1[ j ].rese_ = src[ i ].interface_group_id[ j ].size();
                    md.copy( &loc_1[ j ].data_, src[ i ].interface_group_id[ j ].ptr(), src[ i ].interface_group_id[ j ].size() * sizeof( SI32 ), 16 * 4 );
                }
                md.end_local_data( loc[ i ].interface_group_id.data_, loc_1, rese_1 );
                ...
                ST rese_2 = src[ i ].interface_num_in_group.size() * sizeof( BasicVecRef<SI32 > );
                md.beg_local_data( &loc[ i ].interface_num_in_group.data_, &loc_2, rese_2, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].interface_num_in_group.size(); ++j ) {
                    loc_2[ j ].size_ = src[ i ].interface_num_in_group[ j ].size();
                    loc_2[ j ].rese_ = src[ i ].interface_num_in_group[ j ].size();
                    md.copy( &loc_2[ j ].data_, src[ i ].interface_num_in_group[ j ].ptr(), src[ i ].interface_num_in_group[ j ].size() * sizeof( SI32 ), 16 * 4 );
                }
                md.end_local_data( loc[ i ].interface_num_in_group.data_, loc_2, rese_2 );
                ...
                ST rese_3 = src[ i ].group_interfaces_id.size() * sizeof( BasicVecRef<SI32 > );
                md.beg_local_data( &loc[ i ].group_interfaces_id.data_, &loc_3, rese_3, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].group_interfaces_id.size(); ++j ) {
                    loc_3[ j ].size_ = src[ i ].group_interfaces_id[ j ].size();
                    loc_3[ j ].rese_ = src[ i ].group_interfaces_id[ j ].size();
                    md.copy( &loc_3[ j ].data_, src[ i ].group_interfaces_id[ j ].ptr(), src[ i ].group_interfaces_id[ j ].size() * sizeof( SI32 ), 16 * 4 );
                }
                md.end_local_data( loc[ i ].group_interfaces_id.data_, loc_3, rese_3 );
                loc[ i ].side_N.size_ = src[ i ].side_N.size();
                loc[ i ].side_N.rese_ = src[ i ].side_N.size();
                BasicVecRef<BasicVecRef<BasicVecRef<FP64 > > > *loc_4;
                ST rese_4 = src[ i ].side_N.size() * sizeof( BasicVecRef<BasicVecRef<BasicVecRef<FP64 > > > );
                md.beg_local_data( &loc[ i ].side_N.data_, &loc_4, rese_4, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].side_N.size(); ++j ) {
                    loc_4[ j ].size_ = src[ i ].side_N[ j ].size();
                    loc_4[ j ].rese_ = src[ i ].side_N[ j ].size();
                    BasicVecRef<BasicVecRef<FP64 > > *loc_5;
                    ST rese_5 = src[ i ].side_N[ j ].size() * sizeof( BasicVecRef<BasicVecRef<FP64 > > );
                    md.beg_local_data( &loc_4[ j ].data_, &loc_5, rese_5, sizeof( ST ) );
                    for( ST k = 0; k < src[ i ].side_N[ j ].size(); ++k ) {
                        loc_5[ k ].size_ = src[ i ].side_N[ j ][ k ].size();
                        loc_5[ k ].rese_ = src[ i ].side_N[ j ][ k ].size();
                        BasicVecRef<FP64 > *loc_6;
                        ST rese_6 = src[ i ].side_N[ j ][ k ].size() * sizeof( BasicVecRef<FP64 > );
                        md.beg_local_data( &loc_5[ k ].data_, &loc_6, rese_6, sizeof( ST ) );
                        for( ST l = 0; l < src[ i ].side_N[ j ][ k ].size(); ++l ) {
                            loc_6[ l ].size_ = src[ i ].side_N[ j ][ k ][ l ].size();
                            loc_6[ l ].rese_ = src[ i ].side_N[ j ][ k ][ l ].size();
                            md.copy( &loc_6[ l ].data_, src[ i ].side_N[ j ][ k ][ l ].ptr(), src[ i ].side_N[ j ][ k ][ l ].size() * sizeof( FP64 ), 16 * 4 );
                        }
                        md.end_local_data( loc_5[ k ].data_, loc_6, rese_6 );
                    }
                    md.end_local_data( loc_4[ j ].data_, loc_5, rese_5 );
                }
                md.end_local_data( loc[ i ].side_N.data_, loc_4, rese_4 );
                loc[ i ].side_M.size_ = src[ i ].side_M.size();
                loc[ i ].side_M.rese_ = src[ i ].side_M.size();
                BasicVecRef<BasicVecRef<FP64 > > *loc_7;
                ST rese_7 = src[ i ].side_M.size() * sizeof( BasicVecRef<BasicVecRef<FP64 > > );
                md.beg_local_data( &loc[ i ].side_M.data_, &loc_7, rese_7, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].side_M.size(); ++j ) {
                    loc_7[ j ].size_ = src[ i ].side_M[ j ].size();
                    loc_7[ j ].rese_ = src[ i ].side_M[ j ].size();
                    BasicVecRef<FP64 > *loc_8;
                    ST rese_8 = src[ i ].side_M[ j ].size() * sizeof( BasicVecRef<FP64 > );
                    md.beg_local_data( &loc_7[ j ].data_, &loc_8, rese_8, sizeof( ST ) );
                    for( ST k = 0; k < src[ i ].side_M[ j ].size(); ++k ) {
                        loc_8[ k ].size_ = src[ i ].side_M[ j ][ k ].size();
                        loc_8[ k ].rese_ = src[ i ].side_M[ j ][ k ].size();
                        md.copy( &loc_8[ k ].data_, src[ i ].side_M[ j ][ k ].ptr(), src[ i ].side_M[ j ][ k ].size() * sizeof( FP64 ), 16 * 4 );
                    }
                    md.end_local_data( loc_7[ j ].data_, loc_8, rese_8 );
                }
                md.end_local_data( loc[ i ].side_M.data_, loc_7, rese_7 );
            }
            md.end_local_data( dst, loc, rese );
            return dst;
        }
    ...
    struct GroupInterfacesCompactClass {
        ...
        template<class T> static GroupInterfacesCompactClass *copy( MemoryDriver &md, const T *src, ST num = 1 ) {
            ...
            for( ST i = 0; i < num; ++i ) {
                ...
                loc[ i ].connectivities.rese_ = src[ i ].connectivities.size();
                BasicVecRef<SI32 > *loc_2;
                ST rese_2 = src[ i ].connectivities.size() * sizeof( BasicVecRef<SI32 > );
                md.beg_local_data( &loc[ i ].connectivities.data_, &loc_2, rese_2, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].connectivities.size(); ++j ) {
                    loc_2[ j ].size_ = src[ i ].connectivities[ j ].size();
                    loc_2[ j ].rese_ = src[ i ].connectivities[ j ].size();
                    md.copy( &loc_2[ j ].data_, src[ i ].connectivities[ j ].ptr(), src[ i ].connectivities[ j ].size() * sizeof( SI32 ), 16 * 4 );
                }
                md.end_local_data( loc[ i ].connectivities.data_, loc_2, rese_2 );
                ...
                BasicVecRef<SI32 > *loc_3;
                ST rese_3 = src[ i ].correspondance_between_nodes.size() * sizeof( BasicVecRef<SI32 > );
                md.beg_local_data( &loc[ i ].correspondance_between_nodes.data_, &loc_3, rese_3, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].correspondance_between_nodes.size(); ++j ) {
                    loc_3[ j ].size_ = src[ i ].correspondance_between_nodes[ j ].size();
                    loc_3[ j ].rese_ = src[ i ].correspondance_between_nodes[ j ].size();
                    md.copy( &loc_3[ j ].data_, src[ i ].correspondance_between_nodes[ j ].ptr(), src[ i ].correspondance_between_nodes[ j ].size() * sizeof( SI32 ), 16 * 4 );
                }
                md.end_local_data( loc[ i ].correspondance_between_nodes.data_, loc_3, rese_3 );
                loc[ i ].Ne.size_ = src[ i ].Ne.size();
                loc[ i ].Ne.rese_ = src[ i ].Ne.size();
                BasicVecRef<FP64 > *loc_4;
                ST rese_4 = src[ i ].Ne.size() * sizeof( BasicVecRef<FP64 > );
                md.beg_local_data( &loc[ i ].Ne.data_, &loc_4, rese_4, sizeof( ST ) );
                for( ST j = 0; j < src[ i ].Ne.size(); ++j ) {
                    loc_4[ j ].size_ = src[ i ].Ne[ j ].size();
                    loc_4[ j ].rese_ = src[ i ].Ne[ j ].size();
                    md.copy( &loc_4[ j ].data_, src[ i ].Ne[ j ].ptr(), src[ i ].Ne[ j ].size() * sizeof( FP64 ), 16 * 4 );
                }
                md.end_local_data( loc[ i ].Ne.data_, loc_4, rese_4 );
    ...
    template<class T> static GeometryCompactClass *copy( MemoryDriver &md, const T *src, ST num = 1 ) {
        ...
        for( ST i = 0; i < num; ++i ) {
            ...
            ST rese_0 = src[ i ].nodes.size() * sizeof( BasicVecRef<FP64 > );
            md.beg_local_data( &loc[ i ].nodes.data_, &loc_0, rese_0, sizeof( ST ) );
            for( ST j = 0; j < src[ i ].nodes.size(); ++j ) {
                loc_0[ j ].size_ = src[ i ].nodes[ j ].size();
                loc_0[ j ].rese_ = src[ i ].nodes[ j ].size();
                md.copy( &loc_0[ j ].data_, src[ i ].nodes[ j ].ptr(), src[ i ].nodes[ j ].size() * sizeof( FP64 ), 16 * 4 );
            }
            md.end_local_data( loc[ i ].nodes.data_, loc_0, rese_0 );
