...
struct Sst
{
    ...
    bool maj_operateurs;                /// indique si les operateurs doivent etre reactualises
    ...
    /// Structure pour la definition de l'approximation pli (c.f. mesomodele)
    struct Epaisseur{
        Epaisseur() {}
        Vec<unsigned> element ; /// Elements appartenant a un meme groupe
        TYPEREEL x ;   /// 1ere coordonnee du groupe
        TYPEREEL y ;   /// 2nde coordonnee du groupe
    };
    bool comportement_pli ; /// Homogeneisation des variables internes dans l'epaisseur
    Vec<Epaisseur> pli;             /// Ensemble des Epaisseurs constituant l'approximation pli
    Vec<TYPEREEL,DIM> normale ;     /// Normale au pli dans le cas du mesomodele
