...
**/
template<class T> bool pt_match_equation(Vec<T,2> &pt, string equation, const double eps=1e-3){
    bool flag=false;
...
template<class T> bool pt_match_equation(Vec<T,3> &pt, string equation, const double eps=1e-3){
    bool flag=false;
    std::vector<Ex> symbols;
    symbols.push_back("x");
    symbols.push_back("y");
