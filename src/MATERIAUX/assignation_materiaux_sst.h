...
struct assignation_material_to_SST{
   template<class SST, class TV3> void operator()(SST &S,TV3 &matprop, Param &process) const{
//       S.mesh.load();
      assign_material(S,matprop,process); //fichier genere par __init__.py dans LMT
//       S.mesh.unload();
   }
};
